...
if PROTOCOL_VERSION == 1:
    ...
    ADDR_MX_PROFILE_VELOCITY    = 32    # Address for the profile vel
    ...
    def get_angles(self, radian=False):
        ...
        if PROTOCOL_VERSION == 2:
            ...
            for joint in self.joints:
                servos_angles.append(joint.get_angle(radian=radian))
            return servos_angles
...
class Joint(object):
    ...
    def __init__(self, servo_id, center_value=0,max_value = 2048,max_angle=180):
    ...
    def set_goal_value(self, angle, radian=False):
        ...
        if radian:
            self.goal_value = int(self.max_value*angle/pi) + self.center_value
        else:
            self.goal_value = int(self.max_value*angle/self.max_angle) + self.center_value
        self.changed = True
    ...
    def send_angle(self, angle, radian=False):
        ...
        dxl_comm_result, dxl_error = self.packet_handler.write2ByteTxRx(self.port_handler, self.servo_id, ADDR_MX_GOAL_POSITION, self.goal_value)
    ...
    def get_angle(self, radian=False):
        ...
        if radian:
            self.curr_angle = pi*float(self.curr_value)/self.max_value
        else:
            self.curr_angle = self.max_angle*float(self.curr_value)/self.max_value
    ...
    def send_profile_velocity(self,value=1000):
        ''' Disables torque in this joint
        Usage:
            self.disable_torque()
        '''
        dxl_comm_result, dxl_error = self.packet_handler.write2ByteTxRx(self.port_handler, self.servo_id, 32, value)
