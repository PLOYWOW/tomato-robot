...
class xarm7_move():
    def __init__(self):
        ...
        self.joint_tomato_init = [9.445775503991172e-05, -4.822097616852261e-05, 1.916283872560598e-05, 0.00010871442646021023, -0.00014290939725469798, -0.7803922891616821, 3.711061071953736e-05]
        ...
        rWall_pose.pose.position.y = -0.2
        rWall_pose.pose.position.z = 0.3
        rWall_name = "rWall"
        self.scene.add_box(rWall_name, rWall_pose, size=(0.3, 0.01, 0.6))
    ...
    def executePick(self,tomatoPose):
        ...
        tomato_calibrate_pos_world = [tomatoPickPos_world[0] - 0.13,
                                    tomatoPickPos_world[1],    
                                    tomatoPickPos_world[2] - 0.02]  #gripper Compensate
        ...
        tomatoPrePickPos_world = [tomatoPickPos_world[0] - 0.12,
                            tomatoPickPos_world[1],    
        ...
        time.sleep(2)
        tomatoLiftPos_world = [tomatoPickPos_world[0]-0.05,
                            tomatoPickPos_world[1],    
        ...
        self.tomatoDynamixel.execute("v1g2")
        time.sleep(1.0)
        # self.tomatoDynamixel.execute("v0g2")
        # time.sleep(1.0)
        tomatoHalfCutPos_world = [tomatoLiftPos_world[0]-0.15,
    ...
    def search_tomato(self):
        ...
        tomatoArray = []
        while abs(closet_tomato_y_campos-0.249) > 0.2:
    ...
    def initialize_pick_loop(self):
        ...
        numberOfTomato = len(self.tomatoCenterArray)
        if numberOfTomato < 1 or not self.isFinishBranch:
            tomatoArray = self.search_tomato() #First time use the tomato stop array
        self.isFinishBranch = 0
        for i in range(self.maxNumberPerBranches): 
            if len(self.tomatoCenterArray) < 1:
                self.isFinishBranch = 1
                return 0
            tomatoCenterPose_cam = tomatoArray[0]
            ...
            while time.time() - startTime < runoutTime:
                ...
                if len(self.tomatoCenterArray) <= 0:
                    continue
                checkStablePose_cam = self.tomatoCenterArray[0]
            ...
            # self.tomatoDynamixel.disable_motor(4)
