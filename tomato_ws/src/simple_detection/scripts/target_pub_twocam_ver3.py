...
def main():
    ...
    redArea_pub = rospy.Publisher("red_area_flag",Bool,queue_size=1)
    flag_area = Bool()
    ...
    while not rospy.is_shutdown():
        ...
        # clipping_distance_in_centimeters = trackbar_depth.get_param_as_tuple() #get in cm

        # ##### Print min&max clipping distance ######
        # # if (clipping_distance_in_centimeters[0][0] != previous_min) or (clipping_distance_in_centimeters[1][0] != previous_max):
        ...
        # min_clipping_distance_in_meters = float(clipping_distance_in_centimeters[0][0]) / 100.0  #convert to meter
        # max_clipping_distance_in_meters = float(clipping_distance_in_centimeters[1][0]) / 100.0  #convert to meter

        # min_clipping_distance = min_clipping_distance_in_meters / depth_scale #convert to mm : m*1000 = mm or m/0.001 = mm
        # max_clipping_distance = max_clipping_distance_in_meters / depth_scale #convert to mm : m*1000 = mm or m/0.001 = mm

        # print(min_clipping_distance)
        # print(max_clipping_distance)
        ...
        if(True):       
            ...
            # extracted_image = get_colored_area(left_tomato_cropped_img,Lmask_MIN,Lmask_MAX,Umask_MIN,Umask_MAX)
            ...
            # box_index = np.where(extracted_image != 0)
            # print(box_index)

            # cv2.imwrite("./saved_tomato/tomato.jpg",extracted_image)

            # #apply instance segmentation to red area extracted 
            # segment_ini = segment(extracted_image)
            # seg_extracted_image, seg_extracted_image_transp = segment_ini.instance_segment()

            # cv2.imshow("seg_extracted_image",seg_extracted_image)
            # cv2.imshow("seg_extracted_image_transp",seg_extracted_image_transp)

            #convert red-area to gray scale
            gray_extracted_image = cv2.cvtColor(extracted_image, cv2.COLOR_RGB2GRAY) #w/o segment
            # gray_extracted_image = cv2.cvtColor(seg_extracted_image, cv2.COLOR_RGB2GRAY) #w/ segment
            # cv2.imshow("gray_extracted_image",gray_extracted_image)
            #convert red-area-gray scale to binary img
            ret, threshold_img = cv2.threshold(gray_extracted_image, 1, 255, cv2.THRESH_BINARY)
            # cv2.imshow("binary_img",threshold_img)
            # print(type(ret))
            # print(ret)
            ...
            if(ret):
                ...
                # print("number of contours: ",end="")
                # print(len(contours))
                if(len(contours)>0):
                    #select biggest one
                    # print(type(contours[0]))
                    # print(contours[0].shape)
                    # print(type(contour_sizes)) #list
                    # print("contour_sizes list size ",end="")
                    # print(len(contour_sizes))
                    # print(type(contour_sizes[0]))
                    # list1 = [4,3,6,8]
                    # list2 = max(list1) 
                    # print(list2) #8
                    biggest_contour = max(contour_sizes, key=lambda x: x[0])[1]
                    biggest_contour_index = contour_sizes.index(max(contour_sizes, key=lambda x: x[0]))
                    # print(biggest_contour_index)
                    # print(type(biggest_contour)) #araay
                    # print("biggest_contour shape: ",end="")
                    # print(biggest_contour)
                    # print(biggest_contour.shape)s
                ...
                # cv2.imshow("red extracted image with x,y",extracted_image)
########################################################################################################
                    # #display window
        ...
        #             # depth_pixel = [int(x), int(y)] #center of biggest
        #             # # depth_point = rs.rs2_deproject_pixel_to_point(depth_intrin, depth_pixel, img_depth[depth_pixel[1],depth_pixel[0]]*depth_scale) ##??
        #             #                                             # depth_intrin, depth_pixel, z coor: distance in m
        #             #                                                                         #img_depth: each pixel is distance in mm
        #             #                                                                         #convert distance in mm to m
        ...
        #             cv2.circle(img_bgr, (int(x), int(y)), radius=7, color=(255,0,0), thickness=-1)
        #             pub.publish(center)
        ...
        #         cv2.putText(img_bgr, str(fps), (20, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (77, 255, 9), 2)
        #     # cv2.imshow("bg_removed_BGR", bg_removed)
        #     cv2.imshow("BGR with marker", img_bgr)
        # else:
        #     if (enable_fps_show):
        ...
        # # cv2.imshow("BG Removed minicam image", bg_removed_max)
########################################################################################################
        # cv2.imshow("Final",np.hstack([extracted_image,gray_extracted_image,threshold_img,seg_extracted_image,seg_extracted_image_transp]))
        cv2.namedWindow("1",cv2.WINDOW_NORMAL)
        cv2.resizeWindow("1",(int(640*1),int(360*1)))
        # cv2.resizeWindow("1",(int(1280),int(720)))
        cv2.moveWindow("1",500,0)
        cv2.putText(extracted_image, "with marker", (40, 50), cv2.FONT_HERSHEY_SIMPLEX, 2, (77, 255, 9), 2)
        cv2.imshow("1",extracted_image)
        
        cv2.namedWindow("2",cv2.WINDOW_NORMAL)
        cv2.resizeWindow("2",(int(1280*0.85),int(360*0.85)))
        cv2.moveWindow("2",500,360)
        ...
        # cv2.resizeWindow("3",(int(1280*0.85),int(360*0.85)))
        # cv2.moveWindow("3",500,700)
        # cv2.putText(seg_extracted_image, "segmented", (40, 50), cv2.FONT_HERSHEY_SIMPLEX, 2, (77, 255, 9), 2)
        # cv2.putText(seg_extracted_image_transp, "segmented with transparency", (40, 50), cv2.FONT_HERSHEY_SIMPLEX, 2, (77, 255, 9), 2)
        # cv2.imshow("3",np.hstack([seg_extracted_image_transp,seg_extracted_image]))
