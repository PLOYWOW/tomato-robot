...
class YOLOR(object):
    def __init__(self, 
            model_weights = '/home/hayashi/worksp/camera_ws/src/tomato_detection/scripts/yolor/engine/tomato_w6_fp16.trt',
            #model_weights = 'engine/yolor_csp_x.trt', 
            max_size = 896, 
            #names = 'data/tomato.names'):
            conf = 0.7,
            names = '/home/hayashi/worksp/camera_ws/src/tomato_detection/scripts/yolor/data/tomato.names'):
        self.names = load_classes(names)
        self.colors = [[np.random.randint(0, 255) for _ in range(3)] for _ in range(len(self.names))]
        self.imgsz = (max_size, max_size)
        # Load model
        self.conf = conf
        self.model = TrtModel(model_weights, max_size,total_classes = len(self.names))
...
class DepthImageHandler(object):
    ...
    def publishTomatoPos3Array(self,tomatoPosList: list)-> None:
