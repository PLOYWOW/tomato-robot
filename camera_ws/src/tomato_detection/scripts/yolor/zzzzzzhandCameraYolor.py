...
class DepthImageHandler(object):
    ...
    def colorCallback(self, msg):
        ...
        self.color_image = cv2.cvtColor(color_image, cv2.COLOR_RGB2BGR)
    ...
    def getTomatoMaskImageFromBbox(self,bbox):
        ...
        l_b = np.array([0, 65, 62])
        u_b = np.array([181, 255, 255])
    ...
    def checkHetaAvoidance(self,bbox):
        ...
        centerCheckBlock = self.pixel_crop(tomatoMask,(boxSize//3,boxSize//3),(cX,cY))
        isBlocked = 1 if np.count_nonzero(centerCheckBlock) < 0.95*centerCheckBlock.size else 0

        cv2.circle(self.visual_image,(cX,cY),boxSize//3,(0,255,0),3)
        if isBlocked:
    ...
    def publishTomatoPos3Array(self)-> None:
        ...
        for index in range(tomatoObject.number):
            ...
            quat4 = self.rotationDict[tomatoObject.hetaAction[index]]
