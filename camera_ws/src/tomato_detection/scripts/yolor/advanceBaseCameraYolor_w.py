    ...
    def detect(self, bgr_img,conf=0.7):   
        ...
        if det is not None and len(det):
            ...
            for x1, y1, x2, y2, conf, cls in det:       # x1, y1, x2, y2 in pixel format
                ...
                cv2.circle(visualize_img, (int(x1.item()),int(y1.item())), 5, (255,255,0), -1)
                cv2.circle(visualize_img, (int(x2.item()),int(y2.item())), 5, (255,255,0), -1)
...
class DepthImageHandler(object):
    ...
    def handColorCallback(self,msg):
        color_image = ros_numpy.numpify(msg)
        self.hand_color_image = cv2.cvtColor(color_image, cv2.COLOR_RGB2BGR) #realseense2

        cv2.imshow("hand_color_image",self.hand_color_image)
    ...
    def tomatoDetect(self,color_image,depth_image,isUpdateValue = True):
        ...
        #detected_image,bboxDict = self.model.detect_(color_image) #edit by wave
    ...
    def GetTomato(self):
        ...
        #depth_image[np.where((depth_image > clipping_distance) | (depth_image <= 0))] = 0
        ...
        t2 = time.time()
        FPS = f"FPS:{1/(t2-t1)}"
        cv2.putText(self.visual_image, FPS, (50, 680), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 0), 2, cv2.LINE_4)
        ...
        # detected_image,bboxDict = self.model.detect(hand_color_image)
        # cv2.imshow("handDetection",detected_image)
    ...
    def find_tomato_offset_srv_callback(self,req):
        ...
        hand_color_image = self.hand_color_image
        detected_image,bboxDict = self.model.detect(hand_color_image) #####################################<<<<<<<<<<<<<<<<<<<<<<
        #detected_image,bboxDict = self.model.detect_(hand_color_image) #edit by wave
