...
class DepthImageHandler(object):
    ...
    def GetCropImage(self):
        ...
        # plt.imshow(cropDepthImage)
        # plt.show()
        #clipDepthImage = np.clip(cropDepthImage,0,1000)
        ...
        if self.isCamInfo:
            ...
            self.publishPointCloud(self.cropPointCloudPublisher,self.cropPointCloud)
    ...
    def process(self):
        rospy.init_node('cropPointCloud', anonymous=True)
        #r = rospy.Rate(60)
        ...
        ###publisher
        self.cropPointCloudPublisher = rospy.Publisher('/croppedPointCloud', PointCloud2, queue_size=1)
